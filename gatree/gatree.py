...
class GATree():
    ...
    @staticmethod
    def _predict_and_evaluate(tree, X, y, fitness_function, scope, classes, algorithm):
        """
        ...
        Args:
            ...
            y (pandas.Series): Target values.
            fitness_function (function): Fitness function for the genetic algorithm.
            scope (str): Scope of prediction ratio.
            classes (list): List of classes.
            algorithm (str): Algorithm for decision tree. Can be 'standard' or 'balanced'.

        Returns:
            Node: The evaluated tree.
        """
        # Pass instances through the tree
        for j in range(X.shape[0]):
            tree.predict_one(X.iloc[j], y.iloc[j])

        if algorithm == 'balanced':
            # Calculate, normalise, and apply prediction ratio
            tree.calculate_prediction_ratio(scope, classes)
            tree.apply_prediction_ratio()

            # Initialise predictions
            tree.clear_evaluation()

            # Pass instances through the tree to update predictions
            for j in range(X.shape[0]):
                tree.predict_one(X.iloc[j], y.iloc[j])
    ...
    def fit(self, X, y, population_size=150, max_iter=2000, mutation_probability=0.1, elite_size=1, selection_tournament_size=2, scope='global', algorithm='standard'):
        """
        ...
        Args:
            ...
            scope (str, optional): Scope of prediction ratio. Can be 'global' or 'local'. Global prediction ratio is calculated for the whole tree, while local prediction ratio is calculated for each node.
            algorithm (str, optional): Algorithm for decision tree. Can be 'standard' or 'balanced'.
        ...
        for i in range(max_iter+1):
            ...
            population = Parallel(n_jobs=self.n_jobs)(delayed(GATree._predict_and_evaluate)(
                tree, X, y, self.fitness_function, scope, self.att_values[-1], algorithm) for tree in population)
